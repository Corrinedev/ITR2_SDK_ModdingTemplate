...
public:
    ...
    UFUNCTION(BlueprintCallable)
    static void SaveStringToFile(const FString& InString, const FString& FilePath, const FString& Filename);

    UFUNCTION(BlueprintCallable)
    static void OpenExplorerDirectory(const FString& DirPath);

    UFUNCTION(BlueprintCallable, meta=(WorldContext="WorldContextObject"))
    static void IsTrackingSystemSupported(const UObject* WorldContextObject, const EBPOpenXRControllerDeviceType ControllerType, bool& OutSupportController, const FString& TrackingSystemsName, bool& OutSupportHmd);

    UFUNCTION(BlueprintCallable, BlueprintPure)
    static bool IsActorInitialized(const AActor* Actor);

    static FString GetConsoleCommandHelp(const FString& Command);

    UFUNCTION(BlueprintCallable)
    static TArray<UActorComponent*> GetComponentsWithNoTagsByInterface(const AActor* Actor, const TSubclassOf<UInterface> Interface);

    static UActorComponent* FindComponentWithTagByInterface(const AActor* Actor, const TSubclassOf<UInterface> Interface, const FName& ComponentTag);

    UFUNCTION(BlueprintCallable)
    static void DrawWidgetToRenderTarget(UUserWidget* WidgetToRender, UTextureRenderTarget2D* RenderTarget, const FVector2D DrawSize);

    UFUNCTION(BlueprintCallable)
    static void DrawTextToCanvas(UCanvas* Canvas, const FSlateFontInfo& FontInfo, const FText& RenderText, const FVector2D ScreenPosition, const FVector2D Scale, const FLinearColor RenderColor, const float Kerning, const FLinearColor ShadowColor, const FVector2D ShadowOffset, const bool bCentreX, const bool bCentreY, const bool bOutlined, const FLinearColor OutlineColor);
    ...
    UFUNCTION(BlueprintCallable)
    static void AppendStringToFile(const FString& InString, const FString& Filename);
